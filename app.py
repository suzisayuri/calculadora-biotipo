...
st.sidebar.write("A costura não é apenas técnica, é uma ferramenta de autoestima. Ao entender suas medidas, você para de tentar caber no padrão e faz o padrão caber em você.")import streamlit as st
...
def salvar_lead(email, nome, biotipo_resultado, genero):
    """
    Função Híbrida: Salva nome, email, biotipo e GENERO na planilha.
    """
    try:
        # 1. Define o escopo
        ...
        # 5. Salva (Com Genero na coluna E)
...
if st.button(t["botao"]):
    if not aceite_lgpd:
        ...
        # 2. Chama a função de salvar (COM GENERO)
